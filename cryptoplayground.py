# -*- coding: utf-8 -*-
"""
Created on Sun Dec 19 10:43:08 2021
@author: Christopher D. Yoder

Migrating funcitons from monte.py to here, meant as a module for all. 
"""
...
def BuySellHold(dates, coin_history, current_moneys, pen_rate):
    ...
    *** UNDER DEVELOPMENT
    ...
    REFERENCES
        -
    SYNTAX 
        [move_moneys] = cp.BuySellHold(dates, coin_history, current_moneys, pen_role)
	"""
...
def CoinBaseFees(transaction, **kwargs):
    """
    Oversimplified model of Coinbase fee structure. 
        1. Need to research/test the actual fee structure!
        2. Implement other currencies and payment methods
    INPUTS
        transaction     = amount bought (+VE) or sold (-VE) in USD
    KWARGS 
        currency        = default USD, currency used for calculating 
        method          = default credid, method to make purchase/sell
    OUTPUTS
        fee             = value of fee to be paid to coinbase
    REFERENCES
        https://www.cryptoryancy.com/the-complete-guide-to-coinbase-fees-and-how-to-avoid-them/
    SYNTAX
        [fee] = cp.CoinBaseFees(transaction)
    """
    ...
    methd_default = "ccard"
    curr = ["USD", "BTC"]
    methd = ["ccard", "PayPal", "dcard"]
    if "currency" in kwargs:
        curr_default = kwargs["currency"]
    if "method" in kwargs:
        methd_default = kwargs["method"]
    
    
    
    # calculate fee
    if np.absolute(transaction < feeswap):
        fee = np.absolute(transaction)*lessfee
    else:
        fee = np.absolute(transaction)*morefee
    return fee